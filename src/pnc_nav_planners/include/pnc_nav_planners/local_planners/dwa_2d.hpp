...
#ifndef PNC_NAV_PLANNERS__LOCAL_PLANNERS__DWA_2D_HPP_
#define PNC_NAV_PLANNERS__LOCAL_PLANNERS__DWA_2D_HPP_
...
/**
 * @class DWA2D
 * @brief Dynamic Window Approach 二维局部规划器
 *
 * 在速度空间中采样，前向仿真生成候选轨迹，
 * 通过多目标代价函数选择最优轨迹。
 * 2D 扩展：考虑地形坡度和高度变化。
 */
class DWA2D : public pnc_nav_core::LocalPlannerBase
{
public:
  DWA2D() = default;
  ~DWA2D() override = default;
  ...
  std::string getName() const override { return "DWA2D"; }
...
private:
  ...
  // 代价权重
  double path_distance_bias_{32.0};// 路径距离代价权重
  double goal_distance_bias_{24.0};// 目标距离代价权重
  double obstacle_cost_bias_{0.01};// 障碍物代价权重
};

}  // namespace pnc_nav_planners

#endif  // PNC_NAV_PLANNERS__LOCAL_PLANNERS__DWA_2D_HPP_
